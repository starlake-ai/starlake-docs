...
export default function StructuredData() {
  ...
  const { structuredData } = siteConfig.themeConfig;
  ...
  const mainSchema = structuredData || {
    ...
    about: {
      ...
      description:
        "Open source declarative data pipeline platform for data loading and transformation",
    },
    audience: {
      "@type": "Audience",
      audienceType:
        "Data Engineers, Data Scientists, Data Analytics, AI Engineers, Enterprise IT Teams",
    },
  };
  ...
  const docSchema = currentDoc
    ? {
        ...
        dateModified: currentDoc.lastUpdatedAt
          ? new Date(currentDoc.lastUpdatedAt * 1000).toISOString()
          : undefined,
        author: {
          "@type": "Organization",
          name: "Starlake",
        ...
        articleSection:
          currentDoc.frontMatter?.sidebar_label || currentDoc.title || "",
        isPartOf: {
          "@type": "WebSite",
          url: "https://docs.starlake.ai",
        },
  ...
  return (
    <Head>
      ...
      {currentDoc && currentDoc.id !== "intro" && (
        <script type="application/ld+json">{JSON.stringify(docSchema)}</script>
      )}
    </Head>
  );
}

...
|---|---|---|
| **Extract (EL)** | Built-in zero-code extraction from JDBC/ODBC sources and REST APIs (with auth, pagination, rate limiting, incremental support) | Not included -- requires external tools (Fivetran, Airbyte, etc.) |
...
| **CDC** | Built-in CDC support: push-based (Debezium/Kafka), pull-based (incremental JDBC), and file-based. Merge strategies include UPSERT, SCD2, and DELETE_THEN_INSERT across all engines | Not included -- requires external CDC tools; dbt handles only the transform after data lands |
...
Starlake **automatically analyzes SQL dependencies** and generates ready-to-deploy DAGs for [Airflow](../guides/orchestrate/airflow), [Dagster](../guides/orchestrate/dagster), [Snowflake Tasks](../guides/orchestrate/snowflake-tasks), and other orchestrators. Select a template and Starlake produces the orchestration code.
...
dbt does not handle CDC -- it assumes data has already been extracted and loaded by another tool. Teams typically rely on Fivetran, Airbyte, or Debezium to capture changes before dbt transforms them.

Starlake provides **end-to-end CDC pipelines** in a single tool. It supports [push-based CDC](/starflow/guides/load/cdc) (consuming Debezium events from Kafka), pull-based CDC (incremental JDBC extraction with watermark tracking), and file-based CDC. Changes are merged into target tables using write strategies -- `UPSERT_BY_KEY_AND_TIMESTAMP` for current-state tables, `SCD2` for full change history, or `DELETE_THEN_INSERT` for key-based replacement. Source-and-target deduplication handles multiple events per key within a single batch.
...
- [Orchestration Tutorial](../guides/orchestrate/tutorial) -- Generate and deploy DAGs automatically.
- [Unit Testing](../guides/unit-tests/concepts) -- Test pipelines locally on DuckDB.
- [Starlake Site Builder](../guides/documentation/starlake-site-builder) -- Generate a documentation portal with ERD and lineage.

...
But you can also distribute these models across several containers.
</TabItem>
...
Create models and reference the two jars you uploaded and are now visible to databricks through the `dbfs:/mnt/starlake-app` mount

- The first model (`import`) will copy the files matching the expected patterns into the pending directory for ingestion by starlake 

![starlake import]( /img/databricks/import-model.png "starlake import")

- The second model (`watch`) will run the starlake ingestion job store the result as parquet files in your dataset directory

![starlake watch]( /img/databricks/watch-model.png "starlake watch")

## Ingest your data
Start the `import` model first and then the `watch` model. The execution logs are available through the `runs` tab:

![models runs]( /img/databricks/runs.png "models runs")
...
The audit log for the above models will be stored in a BigQuery table if we set `SL_AUDIT_SINK_TYPE=BigQuerySink` environnment variable 

...
Change Data Capture (CDC) tracks row-level changes (inserts, updates, deletes) in a source database and propagates them to a target. Starlake supports CDC through three patterns that all converge on its [write strategies](/starflow/guides/load/write-strategies) for merging changes into target tables.
...
This pattern uses Starlake's built-in [incremental extraction](/starflow/guides/extract/incremental) to query the source for new or modified rows. No Kafka infrastructure is needed.
...
The `SL_LAST_EXPORT` table tracks every incremental extraction run with watermark values, row counts, and success status. See [Extraction Monitoring](/starflow/guides/extract/monitoring) for details.
...
Yes. Configure a Debezium connector with the `ExtractNewRecordState` SMT to flatten CDC events, then use Starlake's [Kafka ingestion](/starflow/capabilities/ingestion-kafka) to consume them into a raw staging table. A transform task then applies write strategies to merge changes into the target.
...
- [Incremental Extraction](/starflow/guides/extract/incremental) -- configure pull-based CDC with partitionColumn and SL_LAST_EXPORT
- [Kafka Ingestion](/starflow/capabilities/ingestion-kafka) -- consume CDC events from Kafka topics
- [Load Tutorial](./tutorial) -- end-to-end walkthrough for loading files into your warehouse
- [Expectations](/starflow/guides/load/expectations) -- data quality checks for CDC event validation
...
## How to Load JSON Data with Starlake

1. **Infer the JSON schema** -- Run `starlake infer-schema --input-path incoming/<domain>` to detect nested structures and generate a YAML configuration.
2. **Choose the JSON format** -- Set `metadata.format` to `JSON`, `JSON_FLAT` (for flat objects), or `JSON_ARRAY` (for files containing a single array).
3. **Define nested attributes** -- For nested objects, set `type` to `struct`. For arrays, add `array: true`. Define child attributes under the `attributes` key.
4. **Run the load command** -- Execute `starlake load` to parse, validate, and load the JSON data into the target warehouse table.

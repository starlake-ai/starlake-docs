---
title: "Load Strategies — File Ordering"
...
Starlake load strategies control the order in which files are processed during `starlake load` or `starlake autoload`. Two built-in strategies are available: `IngestionTimeStrategy` (chronological order by modification time, the default) and `IngestionNameStrategy` (alphabetical order by file name). You can also implement a custom strategy in Scala by extending the `LoadStrategy` interface.

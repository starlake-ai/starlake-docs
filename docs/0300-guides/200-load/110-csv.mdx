...
| `transform` | SQL transformation to apply to the column value | None |
...
Run `starlake infer-schema --input-path incoming/<domain>`. This generates a `.sl.yml` YAML file with detected column names, types, and parsing options.
...
Yes. Use the `transform` and `privacy` properties on individual attributes, or see the [Transform on load](transform) documentation for adding, replacing, or ignoring attributes.

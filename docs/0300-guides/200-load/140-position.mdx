...
## How to Load Fixed-Width Data with Starlake

1. **Create a reference file** -- Create a text file with one field per line in `name:value` format, where each value has the exact width of the column.
2. **Infer the schema** -- Run `starlake infer-schema --input-path <file> --format FIXED --output-dir metadata/load/<domain>` to generate the YAML configuration with position offsets.
3. **Review column positions** -- Check the generated YAML. Each attribute has `position.first` and `position.last` (0-based, inclusive). Adjust types from `string` to `int`, `date`, etc.
4. **Load the data** -- Execute `starlake load` to parse the fixed-width file and load validated records into the target warehouse.

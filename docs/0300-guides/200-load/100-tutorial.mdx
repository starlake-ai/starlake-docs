...
Starlake loads and validates JSON, XML, and CSV files into your data warehouse in a single step or incrementally using configurable write strategies. The `autoload` command handles format detection, schema inference, staging, and loading automatically. For more control, the manual workflow lets you run `infer-schema`, `stage`, and `load` as separate steps.
...
To query your database, open the DuckDB shell by running the following command:

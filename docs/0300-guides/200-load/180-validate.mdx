---
title: "Type Validation Reference"
description: "Validate data types during ingestion with Starlake. Define schemas with basic and custom types, regex patterns and SQL-mapped type checking."
keywords: [starlake, type validation, data types, schema validation, regex, ingestion]
---
...
Starlake validates every incoming record against a type schema before loading it into the data warehouse. Built-in types cover common SQL primitives (int, double, boolean, timestamp, etc.), and you can define custom types with regex patterns for domain-specific formats such as email, phone or zip code. Records that fail validation are routed to a rejected table for audit.

...
:::tip
Type validation runs in Spark mode only. [Native load mode](/guides/load/native) skips this step entirely. Use post-load [expectations](/guides/load/expectations) as an alternative quality gate when running in native mode.
:::

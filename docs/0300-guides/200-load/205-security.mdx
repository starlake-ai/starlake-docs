...
Starlake provides three levels of security that you can combine in a single table YAML definition: table-level ACL (grant roles to users, groups or service accounts), row-level security (filter rows with SQL predicates per grantee) and column-level access policies (restrict PII columns on BigQuery). All settings are declarative and applied automatically during load.

## How to configure row-level security

1. **Open the table YAML file** -- Edit `metadata/load/<domain>/<table>.sl.yml`.
2. **Add an `rls` section** -- Under the `table` key, define one or more RLS policies.
3. **Set a `name` and `predicate`** -- The predicate is a SQL expression evaluated for each row (e.g., `"country = 'USA'"`).
4. **Define `grants`** -- List the users or groups that should see only the rows matching the predicate (e.g., `"group:mygroup"`).

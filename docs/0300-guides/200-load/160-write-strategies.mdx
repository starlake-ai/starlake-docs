---
title: "Write Strategies — APPEND, OVERWRITE, UPSERT, SCD2"
description: "Choose how Starlake writes data to tables: APPEND, OVERWRITE, UPSERT_BY_KEY, SCD2, DELETE_THEN_INSERT and more write strategies explained."
keywords: [starlake, write strategy, APPEND, OVERWRITE, UPSERT, SCD2, DELETE_THEN_INSERT, merge]
---
...
Starlake provides multiple write strategies that control how incoming data is merged into target tables. Whether you need simple appends, full overwrites, key-based upserts or SCD2 history tracking, the `metadata.writeStrategy` property lets you declare the behavior in YAML. Adaptive mode even allows switching strategies at runtime based on file size, date or file name patterns.

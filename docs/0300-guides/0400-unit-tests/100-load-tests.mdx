...
Test your load model locally before deploying to production.

Load models are the first step in the data pipeline.
They are responsible for loading data from the source to the destination.
In this tutorial, we will test the load models.
...
- one or multiple data files whose names match the file pattern expected by the loader for this table. This is the data that will be loaded into the table using the `starlake load` model.
- a `_expected.csv` file that contains the expected data in the table after the load model is run.

After loading using the `starlake load` model against the schema defined in the `metadata/load` directory,
...
The test will pass if the load model succeeds and the data in the table matches the data in the `_expected.csv` file.
...
- `test-reports/load/test-name/testname.db`: the actual database after the load model is run. This database contains the following tables:
    ...
    - audit.audit: The audit log of the load model.
    - audit.rejected: The rejected data.
- `test-reports/load/test-name/not_expected.csv`: the unexpected data in the actual table after the load model is run.
- `test-reports/load/test-name/missing.csv`: the missing data in the actual table after the load model is run.

The test reports are generated using the `starlake test` model.

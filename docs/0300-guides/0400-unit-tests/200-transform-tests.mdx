...
Transform models are the second step in the data pipeline.
They are responsible for transforming the data in the database.
In this tutorial, we will test the transform models.
...
- a `_expected.csv` or `_expected.sql` file that contains the expected data in the table after the transform model is run.


Before running the transform, starlake will transpile your SQL statements to the Duckdb dialect before running it.
After running the transpiled SQL staement using the `starlake transform` model against local duckdb database populated by starlake using the data files present in the test directory,
...
The test will pass if the transform model succeeds and the data in the table matches the data in the `_expected.csv` file.
...
- `test-reports/transform/test-name/testname.db`: the actual database after the load model is run. This database contains the following tables:
  ...
  - audit.audit: The audit log of the transform model.
- `test-reports/transform/test-name/not_expected.csv`: the unexpected data in the actual table after the transform model is run.
- `test-reports/transform/test-name/missing.csv`: the missing data in the actual table after the transform model is run.

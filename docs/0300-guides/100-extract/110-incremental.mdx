...
Starlake incremental extraction pulls only new rows from a database table by tracking the maximum value of a partition column between runs. The extraction state is persisted in the `SL_LAST_EXPORT` audit table. This mode is configured by setting `fullExport: false` and specifying a `partitionColumn` in the extraction YAML.

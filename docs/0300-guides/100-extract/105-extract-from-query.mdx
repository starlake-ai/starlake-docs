  ...
  <script type="application/ld+json">
    {JSON.stringify({
      ...
      "step": [
        {
          "@type": "HowToStep",
          "name": "Define the extraction YAML with a sql field",
        ...
        {
          "@type": "HowToStep",
          "name": "Set the connection reference",
          "text": "Set connectionRef to a JDBC connection defined in metadata/application.sl.yml.",
          "position": 2
        },
        {
          "@type": "HowToStep",
...
Starlake query extraction lets you export data from a JDBC database using custom SQL statements instead of extracting entire tables. You can apply filters, joins, aggregations, and column selections directly in the YAML configuration. This feature is available for any JDBC connection and is configured by adding a `sql` field to the table entry.

## How to Extract Data from a SQL Query

1. **Define the extraction YAML with a `sql` field** -- In `metadata/extract/`, create a YAML config. In the `tables` section, add a `sql` field with your SQL query. The `name` field serves as the logical output file name.
2. **Set the connection reference** -- Set `connectionRef` to a JDBC connection defined in `metadata/application.sl.yml`.
3. **Run the extract-data command** -- Execute `starlake extract-data --config custom_query_extract --outputDir $SL_ROOT/incoming/`.
4. **Optionally extract the schema** -- Run `starlake extract-schema --config custom_query_extract` to generate a schema description file for warehouse loading.

:::info
Query extraction does not infer primary keys or foreign keys. All attributes are considered not required.
:::

...
Starlake records the result of every extraction run in the `SL_LAST_EXPORT` audit table, including the schema name, table name, row count, success status, and the last exported partition column value. This table is stored in the audit schema (named `audit` by default) and its connection is configured in `metadata/application.sl.yml`. You can query it directly with SQL to detect failures, track volumes, and monitor incremental extraction progress.

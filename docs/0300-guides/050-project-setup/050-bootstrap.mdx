...
The `starlake bootstrap` command initializes a new Starlake project with a standard directory structure. It generates the `metadata/` folder containing configuration files (`application.sl.yml`, environment overrides), type mappings, and sample data in `datasets/incoming/`. The bootstrapped project supports DuckDB, BigQuery, Snowflake, and other connections out of the box.

## How to Create a New Starlake Project (Step-by-Step)

1. **Create an empty project directory** — Run `mkdir $HOME/userguide && cd $HOME/userguide`. This folder will contain the full project structure.
2. **Run starlake bootstrap** — Execute `starlake bootstrap` from the CLI. For Docker: `docker run -v $(pwd):/app/userguide -e SL_ROOT=/app/userguide -it starlakeai/starlake:VERSION bootstrap`.
3. **Review the generated project structure** — The command creates `metadata/` with `application.sl.yml`, environment files, type mappings, and sample data in `datasets/incoming/`.
4. **Configure your data warehouse connection** — Edit `metadata/application.sl.yml` to define connections. Set `SL_ENV` to switch between environments (e.g., `SL_ENV=DUCKDB`).

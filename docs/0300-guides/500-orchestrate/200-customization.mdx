...
- **DAG** **configuration**(s) and their **references** within the loads and models
- **template**(s) that may be customized
- **starlake-orchestration** framework to dynamically generate the models that will be run
- **managing dependencies** between models to execute transforms in the **correct order**
...
| --domains               | optional    | Wether to generate DAG file(s) to load schema(s) or not (*true* by default if *--models* option has not been specified) |
| --models                 | optional    | Whether to generate DAG file(s) for models or not  (*true* by default if *--domains* option has not been specified)     |
...
- at the **transformation** level, in the **transformation** configuration file *$\{SL_ROOT\}/metadata/transform/\{domain\}/\{transformation\}.sl.yml* under the *model.dagRef* property.<br/>
...
The load of the domain will not be conditionned and no pre-load models will be executed (the default strategy).
...
**load_dependencies** defines wether or not we want to **generate recursively** all the **dependencies** associated to each model for which the transformation DAG was generated (*False* by default).
...
#### IStarlakeJob
`ai.starlake.job.IStarlakeJob` is the **generic factory interface** responsible for **generating** the **models** that will run the starlake's [stage](../../cli/stage), [load](../../category/load) and [transform](../../category/transform) commands:

- **sl_import** will generate the model that will run the starlake [stage](../../cli/stage) command.
...
| ------- | ---- | ----------------------------------------------------- |
| task_id | str  | the optional model id (`\{domain\}_import` by default) |
| domain  | str  | the required domain to import                         |

- **sl_load** will generate the model that will run the starlake [load](../../cli/load) command.
...
| name         | type                | description                                                 |
| ------------ | ------------------- | ----------------------------------------------------------- |
| task_id      | str                 | the optional model id (`\{domain\}_\{table\}_load` by default) |
| domain       | str                 | the required domain of the table to load                    |
| table        | str                 | the required table to load                                  |
| spark_config | StarlakeSparkConfig | the optional `ai.starlake.job.StarlakeSparkConfig`        |

- **sl_transform** will generate the model that will run the starlake [transform](../../cli/transform) command.
...
| name              | type                | description                                            |
| ----------------- | ------------------- | ------------------------------------------------------ |
| task_id           | str                 | the optional model id (`{transform_name}` by default) |
...
| name         | type                | description                                           |
| ------------ | ------------------- | ----------------------------------------------------- |
| task_id      | str                 | the required model id                                  |
...
Each **concrete** factory class extends `ai.starlake.airflow.StarlakeAirflowJob` and implements the **sl_job** method that will generate the **Airflow model** that will run the corresponding starlake command.

:::note Default pool

For all templates instantiating `StarlakeAirflowJob` class, the
**default_pool** option defines the Airflow pool to use for all models executed within the DAG.
...
`ai.starlake.airflow.bash.StarlakeAirflowBashJob` is a concrete implementation of `StarlakeAirflowJob` that generates models using `airflow.operators.bash.BashOperator`. Usefull for **on premise** execution.
...
The **creation** of the **Dataproc cluster** can be performed by calling the `create_cluster` method of the *cluster* property or by calling the `pre_tasks` method of the StarlakeAirflowDataprocJob (the call to the `pre_load` method will, behind the scene, call the `pre_tasks` method and add the optional resulting model to the group of Airflow models).
...
#...
    def sl_transform(self, task_id: str, transform_name: str, transform_options: str=None, spark_config: StarlakeSparkConfig=None, **kwargs) -> T:
        """Transform job.
        Generate the scheduler model that will run the starlake `transform` command.

    Args:
            task_id (str): The optional model id.
            transform_name (str): The transform to run.
            transform_options (str): The optional transform options to use.
            spark_config (StarlakeSparkConfig): The optional spark configuration to use.

    Returns:
            T: The scheduler model.
        """
...
# retry_on_failure: True # when asynchronous job execution has been selected, it specifies whether or not we want to use a bash sensor with automatic retry for a specific exit code (implies airflow v2.6+)
    ...
    load_dependencies: False # whether or not to add all dependencies as airflow models within the resulting dag
    default_pool: "custom_default_pool" # pool to use for all models defined within the dag
```
...
For any transformation, Starlake is able to calculate all its dependencies towards other models or loads thanks to the **analysis** of **SQL queries**.

As seen previously, the **load_dependencies** option defines whether or not we wish to recursively **generate all the dependencies** associated with each model for which the transformation DAG must be generated (False by default). If we choose to not generate those dependencies, the corresponding DAG will be scheduled using the **Airflow's data-aware scheduling mechanism**.
...
if load_dependencies.lower() != 'true':
    for model in task_deps:
        _load_datasets(model)
...
Those **required Datasets** are **updated** for **each** **load** and **model** that have been **executed**.
...
Those **required Assets** are **materialized** for **each** **load** and **model** that have been **executed**.

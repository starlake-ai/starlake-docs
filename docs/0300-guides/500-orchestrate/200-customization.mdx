...
[**sl_env_var**](./options#sl_env_var) defines Starlake environment variables passed as an encoded JSON string:
...
[**pre_load_strategy**](./options#pre_load_strategy) conditionally loads the tables of a domain within the DAG. Four strategies are available:
...
[**run_dependencies_first**](./options#run_dependencies_first) controls whether the generator recursively includes all **dependencies** associated with each task in the DAG (*False* by default).

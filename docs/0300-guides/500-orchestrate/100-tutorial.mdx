...
## Running the DAG
Using starlake `dag-generate` command, we can generate a DAG file that will run our load and transform models.
...
or specifically for each load or transform model through the `dagRef` attribute.
...
`load_dependencies: "true"` is a flag that tells the DAG generator to include the dependent tables or models in the DAG file.
...
The load and transforms models will be run as bash commands on the orchestrator. To run them on a different executor, you can change the template to a different one or build your own.

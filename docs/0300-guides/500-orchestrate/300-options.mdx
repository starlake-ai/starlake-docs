...
| **Default** | *(unset &mdash; feature off)* |
| **Required** | No |
...
1. **"Not ready"** &mdash; files haven't arrived yet (IMPORTED/PENDING find zero files, ACK file missing). The DAG should wait and retry.
2. **Real error** &mdash; the pre-load crashed, config is invalid, permissions denied. The DAG should fail fast.
...
1. Appends `--notReadySentinel <resolved-path>` to the Cloud Run container arguments. The Starlake CLI writes a zero-byte marker at that path **only when the pre-load decides files are not yet ready**, and exits `0`. Genuine crashes still exit non-zero and never write the sentinel.
2. After the Cloud Run task succeeds, the Airflow `CloudRunJobCompletionSensor` (or the Dagster Cloud Run op) checks GCS for the sentinel. If present, it deletes the sentinel and raises an exception so the orchestrator's task-retry machinery re-fires the whole TaskGroup after `retry_delay_in_seconds` &mdash; that's how "wait for files" is expressed.
3. If the sentinel is absent, the sensor/op passes and downstream load tasks run.
...
The user supplies only a **parent prefix**. Orchestration automatically appends `<domain>/<run_id>.notready`, producing concrete paths like `gs://my-bucket/_sl/preload/sales/scheduled__2026-04-22T06-00-00.notready`. Domain scoping and per-run uniqueness are non-negotiable &mdash; these prevent two common footguns (cross-domain collisions, concurrent-run collisions). The run id is resolved by Airflow Jinja on the Airflow side and by an explicit `OpExecutionContext.run_id` substitution on the Dagster side.
...
- Add a GCS lifecycle rule on the `_sl/preload/` prefix (e.g., delete after 7 days) as a safety net for orphan sentinels &mdash; the sensor normally deletes on detection, but a failed orchestrator restart could leave one behind.
- The Starlake CLI version running inside your Cloud Run container must support `--notReadySentinel` (introduced alongside this feature). Earlier CLI versions will reject the unknown flag.
...
See the [Dependencies](./orchestrate-advanced-customization#dependencies) section for a detailed explanation of both strategies.
...
`0` means a failed task fails the DAG immediately &mdash; no implicit retries. Set explicitly when you want retry behavior (e.g., when using [`pre_load_not_ready_sentinel_path`](#pre_load_not_ready_sentinel_path), you must set `retries > 0` to provide a "wait for files" window).
...
:::note
For the GCP Cloud Run runner, per-task `retry_delay` is overridden by [`retry_delay_in_seconds`](#gcp-cloud-run) (default `10`s) &mdash; so setting `retry_delay` at the general level has no effect on Cloud Run tasks. Tune `retry_delay_in_seconds` instead.
:::
...
| `retry_delay_in_seconds` | `10` | No | Per-task delay in seconds between Airflow retries for this Cloud Run task. **Overrides the general [`retry_delay`](#retry_delay) option** for Cloud Run tasks. |
| `pre_load_not_ready_sentinel_path` | *(unset)* | No | Opt-in sentinel for pre-load "not ready" signaling. See [the dedicated section](#pre_load_not_ready_sentinel_path). |

...
Add write strategies, partitioning, access control, and more to your transform models.
...
By default, your transforms models will be named after the table they are transforming. The result of the SQL file will be written to the table with the same name as the SQL file (`<table>`in this case) in the schema with the same name as
...
model:
  ...
  name: custom # this model name. Will be used to name the model when running from the CLI
```

...
Starlake transforms can write results not only to database tables but also to files in CSV, JSON, Parquet, or Avro format. You can target local storage, cloud buckets (GCS, S3), or an entirely different database by configuring the `sink` section of the YAML file. Each transform writes to a single destination; to produce multiple outputs, create separate transforms reading the same source.

## How to Export Transform Results

1. **Choose an export format** -- Set `sink.format` in the transform YAML to `csv`, `json`, `parquet`, `avro`, or any Spark-supported format.
2. **Set the file extension** -- Add `sink.extension` to control the output file name extension.
3. **Configure a custom output path** (optional) -- Use `sink.path` to specify a path relative to the `root` defined in `application.sl.yml`. On cloud storage the bucket name is prepended automatically.
4. **Or export to another database** -- Set `sink.connectionRef` to a connection defined in `application.sl.yml`.
5. **Run the transform** -- Execute `starlake transform --name <domain>.<task>` to produce the exported file or write to the target database.

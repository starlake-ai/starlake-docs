...
```bash
starlake lineage  --model kpi.order_summary --print
```
...
If you installed the [GraphViz tool](../../quickstart#prerequisites-3), you can produce the same graph in svg or png format using the following command:

```bash

starlake lineage --model kpi.order_summary --svg --output lineage.svg
```

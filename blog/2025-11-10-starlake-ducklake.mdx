...
**Performance matters just as much as openness.**

For example, in an independent TPC-H SF100 benchmark on Parquet files, 
DuckDB delivered sub-second query times across most queries [DuckDB Benchmark Results](https://www.architecture-performance.fr/ap_blog/tpc-h-benchmark-of-hyper-duckdb-and-datafusion-on-parquet-files/),
demonstrating that open file formats plus a high-performance engine can match many traditional analytics platforms at the fraction of the cost.
With DuckLake adopting the same Parquet + SQL-catalog foundation, and Starlake driving the transformations on top, you’re 
grounded not just in a principled architecture but in real-world, production-scale speed
...
* **Multi-user collaboration**: Unlike local-only formats or single-process metadata layers, DuckLake’s SQL-based catalog allows multiple users, data engineers, analysts, 
applications to read and write concurrently with transactional guarantees.

* **ACID transactions and snapshots**: DuckLake delivers full transactional integrity across datasets, with snapshot isolation, time travel, and schema evolution,
the kind of reliability historically reserved for data warehouses.

* **Open and composable**: Because it’s based on open standards (SQL + Parquet), 
...
* **Local-to-cloud consistency**: You can develop locally with a lightweight DuckDB catalog, then deploy the same datasets on a shared PostgreSQL-backed DuckLake in the cloud, no rewrites, no migrations, no friction.
no rewrites, no migrations, no friction.

## Start Small, Scale Big

Let's start building our data pipeline with Starlake and DuckLake. Full source code is available on [GitHub](https://github.com/starlake-ai/starlake/tree/master/samples/ducklake).

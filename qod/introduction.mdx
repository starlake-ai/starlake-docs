...
QoD is a standalone product; you do not need Starflow to run it. If you use Starflow, the [`starlake quack`](/starflow/cli/quack) command can host a Quack DuckDB server inside the Starflow JVM for local, single-node use, and Starflow can query any Flight SQL endpoint, including a QoD fleet, as a [regular connection](/starflow/guides/project-setup/connections). For multi-tenant fleets with autoscaling and ACLs, use QoD.

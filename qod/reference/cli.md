...
| `NUKE` | `0` | `1` drops the control-plane + demo tenant DBs (needs `psql`) and wipes the local state dirs first. Does NOT wipe the DuckDB cache. Irreversible - on a terminal it asks you to type the control-plane db name before proceeding (`NUKE_YES=1` bypasses; non-tty runs skip the prompt, so scripts and CI are unchanged). |

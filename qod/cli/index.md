...
It is also the launcher: `qod serve --demo` runs the self-contained demo, `qod serve <target>` boots a persistent gateway over your own data (a `.duckdb` file, a parquet/csv directory, or an object-store prefix) on an embedded Postgres, and `qod start` / `qod stop` run a real manager against your own Postgres - see [Installation](/qod/getting-started/install) and the [manager jar reference](/qod/reference/cli). And it bundles the [operator skill for AI agents](/qod/connecting/agent-skill): `qod skill install` teaches Claude Code, Copilot, or Gemini to drive all of the above.
